...
async function callModel(systemPrompt, userContent, maxTokens = 1200, useWebSearch = false, model = "groq", frameworkId = null, groundingMeta = null) {
  ...
  if (!parseJSON(raw)) {
    ...
    try {
      const retried = await callModelOnce(stricter, userContent, maxTokens, useWebSearch, model, frameworkId, groundingMeta);
...
export default function ThinkingOSv2() {
  ...
  // ─── RUN MODE ──────────────────────────────────────────────────────────────
  // New users default to Quick mode (A)
  const [runMode, setRunMode] = useState(() => {
    if (typeof window === "undefined") return "quick";
    const isFirstVisit = !localStorage.getItem("thinking_os_visited");
    return isFirstVisit ? "quick" : "deep";
  });
  const runModeRef = useRef(runMode);
  useEffect(() => { runModeRef.current = runMode; }, [runMode]);

  // ─── FORCED "DON'T DO IT" CASE (D) ────────────────────────────────────────
  const [compareNegated, setCompareNegated] = useState(false);
  ...
  // ─── RUN FULL ANALYSIS (UPDATED WITH ALL IMPROVEMENTS) ─────────────────────
  const runFullAnalysis = useCallback(async (q, category, answers) => {
    ...
    // Helper to run frameworks for a given case (proposed or negated)
    const runFrameworkBatch = async (mode, modeQuestion) => {
      const fwRes = {};
      const fwErrors = [];
      ...
      return { results: fwRes, errors: fwErrors };
    };

    // ─── FORCED "DON'T DO IT" CASE (D) ────────────────────────────────────
    const shouldCompare = compareNegated && !isQuick;

    let proposedResults, negatedResults;
    const proposedQuestion = fullQuestion;
    const negatedQuestion = `Consider this: "NOT doing this." Specifically, analyze the opposite of: "${fullQuestion}"`;

    // Run proposed case
    proposedResults = await runFrameworkBatch('proposed', proposedQuestion);

    // Run negated case if enabled
    let hasNegated = false;
    if (shouldCompare) {
      negatedResults = await runFrameworkBatch('negated', negatedQuestion);
      hasNegated = true;
    }

    // Merge results - if negated exists, we store both separately
    const allFwRes = proposedResults.results;
    const allFwErrors = proposedResults.errors;

    if (hasNegated && negatedResults) {
      // Store negated results separately in col
      col.negatedFrameworks = negatedResults.results;
      // Merge errors
      allFwErrors.push(...negatedResults.errors);
    }

    col.frameworks = allFwRes;
    setCompletedPhases(c => ({ ...c, analysis: true }));
    if (allFwErrors.length > 0) {
      setStageErrors(prev => ({ ...prev, analysis: allFwErrors }));
    }

    const avgFwConf = Object.values(allFwRes).reduce((sum, r) => sum + (r?.confidence || 0), 0) / (Object.keys(allFwRes).length || 1);
    ...
    if (!isQuick && crossData.attacks && crossData.attacks.length > 0) {
      const rebuttalPromises = crossData.attacks.map(async (attack) => {
        ...
        const originalResult = allFwRes[targetFwId];
...
Respond to this attack. Either defend your claim, or concede if the critic is right.`;
      ...
      rebuttals.forEach(r => {
        if (r && allFwRes[r.frameworkId]) {
          allFwRes[r.frameworkId].rebuttal = r.rebuttal;
        }
      });
    }
    ...
    if (isQuick) {
      ...
      try {
        const topRec = crossData?.consensus?.[0]?.recommendation || Object.values(allFwRes)[0]?.recommendation || "proceed with the plan";
    ...
    // ─── RED TEAM FOR NEGATED CASE (D) ──────────────────────────────────────
    let negatedRedData = null;
    if (hasNegated && !isQuick) {
      try {
        const topRecNeg = "Do NOT proceed";
        const raw = await callModel(RED_TEAM_SYSTEM, `Problem: "${negatedQuestion}"\nTop Recommendation: "${topRecNeg}"\n\nRed team this now.`, 1200, false, selectedModel);
        negatedRedData = safeJSON(raw, { failure_modes: [], early_warning_signals: [], risk_severity: [], mitigation_plan: [], kill_shot: "Unknown", survivability: "Conditional", survivability_condition: "" });
      } catch (e) {
        negatedRedData = { failure_modes: [], early_warning_signals: [], risk_severity: [], mitigation_plan: [], kill_shot: "", survivability: "Conditional", survivability_condition: "" };
      }
    }
    ...
    try {
      const payload = {
        ...
        frameworks: Object.entries(allFwRes).map(([id, r]) => compactFramework(id, r)),
    ...
    if (isQuick) {
      ...
      try {
        const payload = {
          ...
          recommendation: crossData?.consensus?.[0]?.recommendation || Object.values(allFwRes)[0]?.recommendation || "No recommendation yet",
        };
    ...
    // ─── SYNTHESIS WITH CONSISTENCY CHECK (F) ──────────────────────────────
    setActivePhase("synthesis");
    
    // Build synthesis payload (includes negated data if available)
    const synthPayload = {
      question: fullQuestion,
      research: compact(col.research),
      reality: compact(col.reality),
    ...
    // Add negated data if available
    let negatedPayload = null;
    if (hasNegated && negatedResults) {
      const negFwRes = negatedResults.results;
      negatedPayload = {
        question: negatedQuestion,
        research: compact(col.research),
        reality: compact(col.reality),
        frameworks: Object.entries(negFwRes).map(([id, r]) => compactFramework(id, r)),
        crossexam: compact(crossData),
        redteam: compact(negatedRedData || {}),
        evidence: compact(evidenceData),
        scenario: compact(scenarioData),
        assumptions: compact(assumptionData),
      };
    ...
    // ─── CONSISTENCY CHECK (F) ──────────────────────────────────────────────
    // Run synthesis 3 times (only in Deep mode)
    let synthesisRuns = [];
    if (!isQuick) {
      for (let i = 0; i < 3; i++) {
        try {
          const questionToAsk = hasNegated && negatedPayload 
            ? `Compare these two cases:\n\nPROPOSED CASE:\n${JSON.stringify(synthPayload)}\n\nNEGATED CASE (NOT doing this):\n${JSON.stringify(negatedPayload)}\n\nWhich is better? Return a recommendation.`
            : `Full analysis:\n${JSON.stringify(synthPayload)}\n\nGenerate final decision output now.`;
          const raw = await callModel(SYNTHESIS_SYSTEM, questionToAsk, 1600, false, selectedModel);
          if (!parseJSON(raw)) flagParseFailure("synthesis", `Synthesis run ${i+1}`);
          const parsed = safeJSON(raw, { status: "ready", recommendation: "Analysis failed — retry.", confidence: 0, confidence_reasoning: [], risk_level: "High", why: [], top_risks: [], what_would_change_positive: [], what_would_change_negative: [], next_actions: [], missing_information: [], recommended_research: [], investigation_needed: true });
          synthesisRuns.push(parsed);
        } catch (e) {
          synthesisRuns.push({ status: "ready", recommendation: `Error: ${e.message}`, confidence: 0 });
        }
      }
    } else {
      // Quick mode: just run once
      try {
        const raw = await callModel(SYNTHESIS_SYSTEM, `Full analysis:\n${JSON.stringify(synthPayload)}`, 1600, false, selectedModel);
        if (!parseJSON(raw)) flagParseFailure("synthesis", "Final synthesis");
        const parsed = safeJSON(raw, { status: "ready", recommendation: "Analysis failed — retry.", confidence: 0, confidence_reasoning: [], risk_level: "High", why: [], top_risks: [], what_would_change_positive: [], what_would_change_negative: [], next_actions: [], missing_information: [], recommended_research: [], investigation_needed: true });
        synthesisRuns = [parsed];
      } catch (e) {
        setStageErrors(prev => ({ ...prev, synthesis: [`Final synthesis failed: ${e.message}`] }));
        synthesisRuns = [{ status: "ready", recommendation: "Synthesis error.", confidence: 0, risk_level: "High" }];
      }
    }

    // Determine consistency
    let finalSynthesis = synthesisRuns[0] || { status: "ready", recommendation: "No result" };
    if (synthesisRuns.length > 1) {
      const recommendations = synthesisRuns.map(r => r.recommendation);
      const uniqueRecs = [...new Set(recommendations)];
      if (uniqueRecs.length === 1) {
        finalSynthesis.consistency = "High";
      } else if (uniqueRecs.length === 2) {
        finalSynthesis.consistency = "Medium";
        finalSynthesis.consistencyWarning = "The model's answer varied between runs – treat with caution.";
      } else {
        finalSynthesis.consistency = "Low";
        finalSynthesis.consistencyWarning = "The model gave different answers each time – this decision is highly uncertain.";
      }
      // Use the first run as the base, but merge consistency fields
      finalSynthesis = { ...synthesisRuns[0], consistency: finalSynthesis.consistency, consistencyWarning: finalSynthesis.consistencyWarning };
    } else {
      finalSynthesis.consistency = "Single Run";
    }

    // ─── CONFIDENCE CALCULATION (E) ─────────────────────────────────────────
    // Mechanically calculate confidence from evidence
    let carriedConfidence = avgFwConf || 0;
    const contradictingCount = evidenceData.contradicting_evidence?.length || 0;
    carriedConfidence -= contradictingCount * 5; // -5% per contradicting piece
    const missingCount = evidenceData.missing_evidence?.length || 0;
    carriedConfidence -= missingCount * 3; // -3% per missing piece
    const unverifiedCount = assumptionData?.summary?.unverified || 0;
    carriedConfidence -= unverifiedCount * 2; // -2% per unverified assumption
    if (redData.survivability === "No") carriedConfidence -= 20;
    if (redData.survivability === "Conditional") carriedConfidence -= 10;
    carriedConfidence = Math.max(0, Math.min(100, Math.round(carriedConfidence)));

    // Overwrite the model's confidence with our mechanical calculation
    finalSynthesis.confidence = carriedConfidence;

    // Add mechanical confidence reasoning
    finalSynthesis.confidence_reasoning = [
      `Started with average framework confidence of ${Math.round(avgFwConf)}%`,
      contradictingCount > 0 ? `Deducted ${contradictingCount * 5}% for ${contradictingCount} contradicting evidence pieces` : null,
      missingCount > 0 ? `Deducted ${missingCount * 3}% for ${missingCount} missing evidence pieces` : null,
      unverifiedCount > 0 ? `Deducted ${unverifiedCount * 2}% for ${unverifiedCount} unverified assumptions` : null,
      redData.survivability === "No" ? "Deducted 20% because Red Team said it wouldn't survive" : null,
      redData.survivability === "Conditional" ? "Deducted 10% because Red Team said survival is conditional" : null,
      `Final confidence: ${carriedConfidence}%`
    ].filter(Boolean);

    // Store in col
    col.synthesis = finalSynthesis;
    setPhaseData(p => ({ ...p, synthesis: finalSynthesis }));
    setCompletedPhases(c => ({ ...c, synthesis: true }));

    // Store proposed and negated results in phaseData if available
    if (hasNegated && negatedResults) {
      const negFwRes = negatedResults.results;
      const negAvgConf = Object.values(negFwRes).reduce((sum, r) => sum + (r?.confidence || 0), 0) / (Object.keys(negFwRes).length || 1);
      const negEvidence = evidenceData; // reuse
      const negRed = negatedRedData || { survivability: "Conditional" };
      let negConfidence = negAvgConf || 0;
      const negContradicting = negEvidence.contradicting_evidence?.length || 0;
      negConfidence -= negContradicting * 5;
      const negMissing = negEvidence.missing_evidence?.length || 0;
      negConfidence -= negMissing * 3;
      if (negRed.survivability === "No") negConfidence -= 20;
      if (negRed.survivability === "Conditional") negConfidence -= 10;
      negConfidence = Math.max(0, Math.min(100, Math.round(negConfidence)));

      setPhaseData(p => ({ 
        ...p, 
        proposed: { synthesis: finalSynthesis, frameworks: allFwRes, redteam: redData },
        negated: { 
          synthesis: { ...finalSynthesis, recommendation: "Do NOT proceed", confidence: negConfidence }, 
          frameworks: negFwRes, 
          redteam: negRed 
        }
      }));
    ...
    // ─── JOURNAL REMINDER (C) ─────────────────────────────────────────────────
    // After synthesis, ask for check-in date
    if (finalSynthesis.status === "ready") {
      const days = prompt(
        "📅 When will you know if this decision worked out?\n\n" +
        "Enter number of days (e.g., 7, 30, 90, or custom):\n" +
        "(We'll remind you to log the outcome at that time)"
      );
      let checkInDate = null;
      if (days && !isNaN(days) && parseInt(days) > 0) {
        checkInDate = new Date();
        checkInDate.setDate(checkInDate.getDate() + parseInt(days));
      }
      setPendingEntry(prev => {
        const updated = { ...prev, checkInDate: checkInDate?.toISOString() || null };
        // If we already have a pending entry, update it
    ...
    const trace = {
      ...
      framework_trace: fws.map(fw => ({ framework: fw.id, label: fw.label, selected: true, reason: "Selected by Reality Extraction", input: "Provided facts and assumptions", output: allFwRes[fw.id] || {}, insights: allFwRes[fw.id]?.key_claim || "No key claim" })),
      ...
      alternatives_considered: (finalSynthesis?.what_would_change_positive || []).map(alt => ({ description: alt, pros: "Could improve outcome", cons: "Not evaluated", reason_rejected: "Not chosen" })),
      final_decision: { recommendation: finalSynthesis?.recommendation || "", reasoning_summary: finalSynthesis?.why?.join("; ") || "", trade_offs: finalSynthesis?.what_would_change_negative || [], remaining_risks: finalSynthesis?.top_risks || [], expected_outcome: finalSynthesis?.next_actions?.[0] || "Awaiting execution" },
      statistics: { frameworks_used: fws.length, sources_consulted: researchData?.sources?.length || 0, evidence_count: researchData?.facts?.length || 0, assumptions: realityData?.assumptions?.length || 0, challenges_raised: crossData?.major_disagreements?.length || 0, risks_identified: redData?.failure_modes?.length || 0, alternatives_evaluated: finalSynthesis?.what_would_change_positive?.length || 0, total_processing_time_ms: endTime - startTime },
      scenario_simulation: {
    ...
    setPendingEntry(prev => {
      const base = {
        ...
        confidence: carriedConfidence || 0,
        risk_level: finalSynthesis?.risk_level || "Unknown",
        problem_type: category,
        reasoning: finalSynthesis?.why?.join("; ") || "",
        framework_ids: fws.map(f => f.id),
        outcome: null,
        accuracy: null,
        checkInDate: prev?.checkInDate || null,
    ...
    setInfoStatus("");
    setActiveTab("synthesis");
    // Set first visit flag so next time they get Deep mode by default (A)
    if (typeof window !== "undefined") {
      localStorage.setItem("thinking_os_visited", "true");
    }
  }, [scores, manualProblemType, selectedFwIds, traces, selectedModel, compareNegated]);
        ...
        <div style={{ padding: "30px 20px 0", flexShrink: 0 }}>
          <div style={{ background: "#ffffff", borderRadius: "14px", padding: "24px", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "12px", marginTop: "16px", alignItems: "center" }}>
              ...
              {/* ─── FORCED "DON'T DO IT" TOGGLE (D) ───────────────────────────── */}
              {runMode === "deep" && (
                <label style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "12px", color: "#4a5568", cursor: "pointer" }}>
                  <input
                    type="checkbox"
                    checked={compareNegated}
                    onChange={e => setCompareNegated(e.target.checked)}
                    style={{ width: "16px", height: "16px", cursor: "pointer" }}
                  />
                  🔥 Compare "Don't do it" case <span style={{ fontSize: "10px", color: "#94a3b8" }}>(doubles analysis time)</span>
                </label>
              )}
                      ...
                      {insufficientInfo ? (
                        ...
                        <>
                          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "12px", marginBottom: "12px" }}>
                            <div style={{ flex: 1 }}>
                              <div style={{ fontSize: "12px", color: "#718096", fontWeight: "600", letterSpacing: "0.1em", marginBottom: "4px" }}>
                                FINAL DECISION
                                {synthesis.consistency && (
                                  <span style={{ marginLeft: "10px", fontSize: "11px", background: synthesis.consistency === "High" ? "#22c55e20" : synthesis.consistency === "Medium" ? "#f59e0b20" : "#ef444420", padding: "2px 8px", borderRadius: "12px", color: synthesis.consistency === "High" ? "#22c55e" : synthesis.consistency === "Medium" ? "#f59e0b" : "#ef4444" }}>
                              ...
                              {synthesis.consistencyWarning && (
                                <div style={{ fontSize: "12px", color: "#c05621", marginTop: "6px" }}>⚠️ {synthesis.consistencyWarning}</div>
                              )}
                              ...
                              {/* Show proposed vs negated comparison if available */}
                              {phaseData.proposed && phaseData.negated && (
                                <div style={{ marginTop: "8px", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
                                  <div style={{ background: "#f0fdf4", padding: "8px 12px", borderRadius: "6px", border: "1px solid #86efac" }}>
                                    <div style={{ fontSize: "11px", fontWeight: "600", color: "#166534" }}>✅ Proposed</div>
                                    <div style={{ fontSize: "13px", color: "#1a1a2e" }}>{phaseData.proposed.synthesis?.recommendation}</div>
                                    <div style={{ fontSize: "12px", color: "#718096" }}>Confidence: {phaseData.proposed.synthesis?.confidence}%</div>
                                  </div>
                                  <div style={{ background: "#fef2f2", padding: "8px 12px", borderRadius: "6px", border: "1px solid #fca5a5" }}>
                                    <div style={{ fontSize: "11px", fontWeight: "600", color: "#991b1b" }}>❌ Negated (Don't do it)</div>
                                    <div style={{ fontSize: "13px", color: "#1a1a2e" }}>{phaseData.negated.synthesis?.recommendation}</div>
                                    <div style={{ fontSize: "12px", color: "#718096" }}>Confidence: {phaseData.negated.synthesis?.confidence}%</div>
                                  </div>
                                </div>
                              )}
                            </div>
            ...
            {activeFrameworkId && !activeFrameworkId.startsWith("__") && activeFw && (
              <div style={{ marginTop: "20px", background: "#ffffff", border: `1px solid ${activeFw.color}40`, borderRadius: "12px", padding: "16px 20px", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "12px" }}>
                  ...
                  <div><div style={{ fontSize: "16px", fontWeight: "700", color: "#1a1a2e" }}>{activeFw.label}</div><div style={{ fontSize: "12px", color: "#718096" }}>{activeFw.thinker}</div><GroundingBadge fwId={activeFw.id} result={activeFwResult} /></div>
